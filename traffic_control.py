...
class TrafficController:
    def __init__(self, lane_count):
        ...
        self.yellow_time = 5  # Updated to 5 seconds yellow time
    ...
    def control_traffic_lights(self, system):
        """Control traffic lights with mandatory yellow signal transition"""
            ...
            try:
                ...
                for i in range(self.lane_count):
                    if 'emergency' in self.lane_vehicle_counts[i] and self.lane_vehicle_counts[i]['emergency'] > 0:
                        ...
                        highest_priority_lane = i
                        break
                
                # Mandatory yellow signal transition
                if self.current_green_lane is not None and self.current_green_lane != highest_priority_lane:
                    # Change current green lane to yellow
                    self.traffic_states[self.current_green_lane] = 'yellow'
                    
                    # Update GUI for yellow state
                    if hasattr(system, 'gui'):
                        system.gui.update_traffic_indicator(self.current_green_lane)
                    
                    # Mandatory 5-second yellow signal
                    print(f"Yellow Signal: Lane {self.current_green_lane + 1}")
                    time.sleep(self.yellow_time)  # 5-second yellow phase
                
                # All lanes red for safety
                self.traffic_states = ['red'] * self.lane_count
                for i in range(self.lane_count):
                    if hasattr(system, 'gui'):
                        system.gui.update_traffic_indicator(i)
                time.sleep(0.5)  # Short all-red safety period
                ...
                # Log lane switch details
                print(f"Green Signal: Lane {highest_priority_lane + 1} with priority {highest_priority:.2f}")
